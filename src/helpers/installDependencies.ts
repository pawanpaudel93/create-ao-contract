...
export const installDependencies = async ({ projectDir }: { projectDir: string }) => {
  ...
  if (isDependenciesInstalled) {
    ...
    (installSpinner ?? ora()).fail(
      chalk.red(
        "Failed to install dependencies! Please refer to https://github.com/pawanpaudel93/create-ao-contract#usage for troubleshooting steps to resolve this issue.\n"
      )
    );
